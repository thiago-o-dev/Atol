...
{
    public static Vector3 GerstnerWave(Vector3 position, Vector2 direction, float steepness, float wavelength, float speed, float timeSinceStart)
    {
        ...
        return new Vector3(
            normalizedDirection.x * a * Mathf.Cos(f),
            a * Mathf.Sin(f),
            normalizedDirection.y * a * Mathf.Cos(f));
    }
}
